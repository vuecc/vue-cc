...
*/
window.CryptoJS = window.CryptoJS || function (q, i) {
  var h = {},
    j = h.lib = {},
    p = j.Base = function () {
      function a() {}
    ...
    l = j.WordArray = p.extend({
      ...
      concat: function (a) {
        ...
        this.clamp();
        if (g % 4)
          for (var e = 0; e < a; e++) c[g + e >>> 2] |= (b[e >>> 2] >>> 24 - 8 * (e % 4) & 255) << 24 - 8 * ((g + e) % 4);
        else if (65535 < b.length)
          for (e = 0; e < a; e += 4) c[g + e >>> 2] = b[e >>> 2];
  ...
  q.enc.Base64 = {
    stringify: function (h) {
      ...
      for (var h = [], k = 0; k < i; k += 3)
        for (var r = (j[k >>> 2] >>> 24 - 8 * (k % 4) & 255) << 16 | (j[k + 1 >>> 2] >>> 24 - 8 * ((k + 1) % 4) & 255) << 8 | j[k + 2 >>> 2] >>> 24 - 8 * ((k + 2) % 4) & 255, o = 0; 4 > o && k + 0.75 * o < i; o++) h.push(l.charAt(r >>> 6 * (3 - o) & 63));
      if (j = l.charAt(64))
        for (; h.length % 4;) h.push(j);
      return h.join("")
    },
    parse: function (h) {
      var h = h.replace(/\s/g, ""),
...
window.CryptoJS.lib.Cipher || function (q) {
